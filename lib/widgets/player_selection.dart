...
class _PlayerSelectionState extends State<PlayerSelection> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_filteredPlayers == null) {
      _filteredPlayers = List.from(data.players.values);
    }
    User user = data.users[data.currentUser.userId];
    if (widget.hidePinned) {
      _filteredPlayers = _filteredPlayers.where((p) => !user.pinnedPlayerIds.contains(p.playerId)).toList();
    }
      ...
      body: ListView.builder(
          ...
          itemBuilder: (context, index) {
            if (index == 0) {
                ...
                child: Row(
                  children: <Widget>[
                    ...
                    OutlineButton.icon(
                      ...
                      onPressed: () {
                        _newPlayer(context, user, _filterText);
                      },
                    )
                  ],
                ),
              ...
              Player player = _filteredPlayers[index - 1];
              return ListTile(
                dense: true,
                title: Text(
                  player.fullName,
  ...
  void _newPlayer(BuildContext context, User user, String initialName) {
